...
The `bestFit` Modifier is used in Line Charts to generate a line of best fit using linear regression. The regression is computed against index positions (0, 1, 2, ...) as the X values and your data values as the Y values.
...
The `bestFitNumber` Optional modifier is used with the `bestFit` modifier to specify which series to compute the best fit line for.
...
The `stepped` Modifier is used in Line Charts to render the line in a stepped fashion. When enabled, the line will render as a staircase rather than a smooth or straight diagonal between points.

- Expected: `boolean` (`true` or `false`) or `string` (`'before'`, `'after'`, `'middle'`)
- Default: `false`
...
## `labelColors` Modifier

The `labelColors` Modifier is used to assign colors based on labels instead of series. This is most useful for Pie, Doughnut, and Polar Area Charts.
...
The `padding` Modifier adds padding around the chart.
...
The `textColor` Modifier overrides the text color for chart labels and legends.

- Expected: `string` (CSS color value)
- Default: Obsidian's `--text-muted` CSS variable

## `rMin` and `rMax` Modifiers

Set the minimum and maximum values for the radial axis in Radar and Polar Area Charts.
